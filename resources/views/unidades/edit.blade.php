...
@section('content')
<div class="mb-6">
    <h2 class="text-3xl font-bold">Editar Unidade</h2>
</div>
    ...
    <form action="{{ route('unidades.update', $unidade) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="cidade" value="{{ old('cidade', $unidade->cidade) }}" required
                    class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('cidade')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="quantidade_vagas" value="{{ old('quantidade_vagas', $unidade->quantidade_vagas) }}" required min="{{ $unidade->vagasOcupadas() }}"
                    class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                <p class="text-xs text-gray-500 mt-1">Mínimo: {{ $unidade->vagasOcupadas() }} (vagas já ocupadas)</p>
            ...
            <div>
                ...
                <input type="number" step="any" name="latitude" value="{{ old('latitude', $unidade->latitude) }}"
                    class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('latitude')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" step="any" name="longitude" value="{{ old('longitude', $unidade->longitude) }}"
                    class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('longitude')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
